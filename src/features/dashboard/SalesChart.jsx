...
function SalesChart({ bookings, numDays }) {
  ...
  // Generate Date Range:
  // Uses eachDayOfInterval from the date-fns library to create an array of dates.
  const allDates = eachDayOfInterval({
    start: subDays(new Date(), numDays - 1),
    end: new Date(),
  });

  // Maps over each date in the generated date range and for each date, it creates an object.
  const data = allDates.map((date) => {
    return {
      label: format(date, "MMM dd"),
      // Sum of totalPrice from bookings on that date
      totalSales: bookings
        .filter((booking) => isSameDay(date, new Date(booking.created_at)))
        .reduce((acc, cur) => acc + cur.totalPrice, 0),
      // Sum of extrasPrice from bookings on that date
      extrasSales: bookings
        .filter((booking) => isSameDay(date, new Date(booking.created_at)))
        .reduce((acc, cur) => acc + cur.extrasPrice, 0),
    };
  });
  // Filters bookings to include only those with created_at matching the current date.
  // Uses reduce to sum up totalPrice and extrasPrice for the filtered bookings.
  // Produces an array of objects containing sales data for each date in the specified range.
  ...
  return (
    <StyledSalesChart>
      ...
      Wrap the chart in a ResponsiveContainer to make it fluid and responsive.
      ...
      <ResponsiveContainer height={300} width="100%">
        <AreaChart data={data}>
          ...
          <YAxis
            ...
            tickLine={{ stroke: colors.text }}
          />
          {/* Style lines using props like strokeDashArray */}
